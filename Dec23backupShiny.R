...
cheque_viz <- cheque_viz%>%
  ...
  group_by(Date)%>%
  mutate(Total=round(sum(Pur+CC)))%>%
  mutate(Pur_Prop=round(Pur/(Pur+CC),2)*100,CC_Prop=round(CC/(Pur+CC),2)*100)%>%
  ungroup()

cheq_viz_join <- cheque_viz%>%
  group_by(Date)%>%
  mutate(
    X50=round(get('Pur_Prop')),
    X100=round(get('CC_Prop'))
  )%>%
  arrange(-X100)%>%
  summarise(Salary = list(c(X50,X100)))
...
body <- dashboardBody(
  mainPanel(
    tabsetPanel(type="tabs",
                ...
                tabPanel("Cheque Details",value=6,gt_output(outputId = "cheque"),height = px(800),
                         width = px(800)),
                id="tabselected"
    )
  )
...
server <- function(input,
  ...
  gt_tbl4 <-reactive({
    cheque_viz%>%
      ...
      select(Date,"Purchase"=Pur,"Cheque_Cleared"=CC,Total,Salary)%>%
  ...
  output$acc2 <-
    renderDT({
      DT::datatable( chrs_acc2, caption = "Cheques cleared",options = list(
        pageLength=50, scrollY='400px'), filter = 'top')
    })
    ...
    render_gt({
      gt_tbl4() %>%
        ...
        gt_plt_bar_stack(
          ...
          position = 'stack', labels = c("Purchase %","","Cheque_Cleared %"),
          width = 80,trim=FALSE
        )%>%
        tab_header(
          title = paste0("Purchase vs Cheque Cleared : Updated Till " ,max(cheque_viz$Date)),
